...
| Lv | 状態 | 判定基準 | 時期目標 |
|---|---|---|---|
| **Lv1** | AIリテラシー | **G検定合格** | 2026/9（7/7からインプット） |
...
現在地: **Lv1の手前・学習未着手**（教材＝5分野の物語ノートとカードは整備済み。7/7からインプット開始）。
...
```mermaid
flowchart LR
  P0["P0 いま〜11/7<br/>G検定＋3B1B"] --> P1["P1 11/8〜12月中旬<br/>数学リブート×NumPy"]
  P1 --> P2["P2 12月中旬〜2月<br/>E資格認定プログラム"]
  P2 --> P3["P3 2〜4月<br/>E資格直前対策"]
  P3 --> P4["P4 5〜10月<br/>情報工学の体系学習"]
...
### P0: 〜2026/11/7 — G検定＋LLM直感（実行中・第5回を見送り、第6回へ）
- 内容は日次プラン v7 の通り（3B1B全7章 × G検定インプット10コマ→演習→スプリント）。
- 🎯 出口条件: G検定合格＋「NNからLLMまで」を自分の言葉で説明できる。

### P1: 11/8〜12月中旬（約6週）— E資格の前提固め【開発解禁】
...
- **認定プログラム選定・申込**（12月上旬まで）: 質重視・会社補助前提。候補: スキルアップAI（認定第1号）／AVILEN／キカガク。無料説明会で「質問対応の質」「修了要件の重さ」を比較して決定。
- 🎯 出口条件: 3層NNをNumPyだけで書いて MNIST 相当を学習させられる／認定プログラム申込完了。

### P2: 2026/12中旬〜2027/2 — 認定プログラム受講（学習の主軸を明け渡す）
- この期間は**認定プログラムのカリキュラムが主、このサイトは記録係**。60分/日の大半をプログラム消化に充てる。
- G検定の維持復習は週1・15分（ひっかけ集の流し読み）まで縮小。
- 🎯 出口条件: 修了要件クリア（＝E資格受験資格の取得）。

### P3: E資格直前対策 — 既定線は 2027/6〜8（8月回）
- 定番問題集（通称・黒本）＋プログラム付属の模試を回す。シラバス改定点を公式で確認。
- **2月回を狙う場合**は 2027/1〜2 に前倒しになるが、下のリスク節のとおり現状は厳しい。
- 🎯 出口条件: **E資格合格（Lv3）**。

### P4: 2027/5〜10月 — 情報工学の体系学習（資格は使わない）
...
**前提が変わった: G検定が第5回(9/5)→第6回(11/7)にずれた。**
これにより後続が約2か月後ろへ動き、**E資格2027年2月回は実質的に届かない**。
認定プログラムに入れるのが12月中旬、標準受講期間は3〜6か月。2月下旬の本番までは約10週しかなく、
修了要件（＝受験資格）に間に合うかどうかの勝負になり、直前対策の時間は残らない。

**したがって既定線を「8月回」に置き直す。** 2月回は「前倒しできたら狙う」に格下げする。
スライドは失敗ではない。8月回に移すと P4（情報工学）が先に来るだけで、最終ゴール（Lv5）への道は変わらない。
むしろ第2章（電子回路からOSまで）を先に消化できるぶん、P4 の助走になる。

| ゲート | 時期 | 判定 | 判断 |
|---|---|---|---|
| **G0** | 2026/10/29 | G検定 第6回に申し込んだ？ | **未達なら次回へ。ここは落とさない** |
| **G1** | 2026/12上旬 | 認定プログラム申込済み？ P1出口条件クリア？ | 未達なら P1 を2週延長 |
| **G2** | 2027/2上旬 | プログラム進捗が計画の8割以上？ | 未達なら P4 を先に前倒しし、8月回に集中 |
| **G3** | 2027/6中旬 | 模試で合格圏？ | 未達なら次回へ。学習は止めない |
| 常時 | — | 週の消化率が2週連続50%未満 | 計画を圧縮再編（内容を削る。日数は増やさない） |

その他のリスク：
- **数学でつまずく** → 原則2（実装とペア）に立ち返る。式で分からなければコードで動かす。
- **燃え尽き** → 60分を死守し増やさない。日曜は15分のまま。
- **2027年のE資格日程が未発表** → 8月回を仮置き。**12月にJDLA公式で確定日を確認**（P1のタスクに含める）。
...
| 3 | 日次プランの「Phase2＝API実装」 | E資格ロードマップと不整合 | ✅ 「P1＝数学×NumPy」に書き換え済み |
